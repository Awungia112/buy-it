...
export default async function AdminCustomersPage() {
  ...
  const activeCustomers = customersWithStats.filter(customer => customer.totalOrders > 0).length;
    ...
    <AdminLayout title="Customers Management">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div className="flex flex-col gap-2 rounded-lg p-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-black dark:text-white text-3xl font-bold">{totalCustomers}</p>
        </div>
        <div className="flex flex-col gap-2 rounded-lg p-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
          <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">Active Customers</p>
          <p className="text-black dark:text-white text-3xl font-bold">{activeCustomers}</p>
        </div>
        <div className="flex flex-col gap-2 rounded-lg p-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
          ...
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead className="text-xs text-gray-700 dark:text-gray-300 uppercase bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-6 py-3" scope="col">Orders</th>
                <th className="px-6 py-3" scope="col">Total Spent</th>
                <th className="px-6 py-3" scope="col">Last Order</th>
                <th className="px-6 py-3" scope="col">Joined</th>
      ...
      {customersWithStats.length > 0 && (
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
            <h3 className="text-black dark:text-white font-semibold mb-4">Top Customers</h3>
                ...
                .map((customer, index) => (
                  <div key={customer.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-black dark:text-white">{customer.name || customer.email}</span>
                    </div>
                    <span className="font-medium text-black dark:text-white">${customer.totalSpent.toFixed(2)}</span>
                  </div>
                ))}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
            <h3 className="text-black dark:text-white font-semibold mb-4">Recent Signups</h3>
            <div className="space-y-3">
              {customersWithStats
                  ...
                  <div key={customer.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                        <span className="material-symbols-outlined text-sm text-gray-600 dark:text-gray-400">person</span>
                      </div>
                      <div>
                        <p className="text-black dark:text-white text-sm">{customer.name || 'N/A'}</p>
